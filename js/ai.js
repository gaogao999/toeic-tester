...
const AI = (() => {
  ...
  function adaptiveReason(a) {
    const rates = (a['難易度ごとの正答'] || []).map((r) => `${r.難易度} ${r.正答}/${r.出題}`).join(' ／ ');
    switch (a['測り方']) {
      // 階段は上がっても、その段で7割取れていなければ到達とは言わない。
      // 4択のまぐれ当たりでも難易度は上がるので、正答率のほうを信じる
      case '正答率が届いていない':
        return `難易度は ${a['最高到達難易度']} まで上がりましたが、そこで7割を取れていません（${rates}）。正答率から ${a['推定レベル']} と見ています。`;
  ...
  const SYSTEM_PROMPT = [
    ...
    '- 「正答率で下げた」が true の回は、難易度は上がったがその段で7割を取れていない。',
