/**
 * intermediate-draft.json（本文・設問・正解）と annotations-intermediate.json
 * （日本語のタイトル・解説・語注）を合流させて、js/reading-data.js の
 * READING_DATA 末尾に r59 以降のエントリとして追記する。
 *
 * 仕組みと注意点は tools/emit-reading-basic.mjs と同じ（二重追記の検知、
 * words の実数カウント、注釈なしページのスキップ報告）。
 *
 * 使い方: node tools/emit-reading-intermediate.mjs
 */
import { readFileSync, writeFileSync } from 'node:fs';

const DRAFT = 'data/raw/reading-import/intermediate-draft.json';
const NOTES = 'data/raw/reading-import/annotations-intermediate.json';
const TARGET = 'js/reading-data.js';
const FIRST_ID = 59; // r58 までは既存（Basic 分）
const LEVEL = 3; // Intermediate = B1
